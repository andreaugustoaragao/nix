...
let
  ...
  # Default URL changed from "https://crates.io/api/v1/crates" to
  # "https://static.crates.io/crates": crates.io now returns 403 for
  # any non-`cargo/*` user-agent (see https://crates.io/data-access),
  # and nixpkgs's fetchurl uses curl's default UA. The static.crates.io
  # CDN is the host the api endpoint already redirects to and serves
  # any UA. This is the only deviation from the upstream nixpkgs file —
  # drop the override once nixpkgs upstreams the same change.
  registries = {
