...

{
  ...
  # System-level auto-rebuild: watch the working tree and rebuild on
  # any .nix file change. Disabled — runs nixos-rebuild as root on any
  # write under the config dir, which is both a privilege-escalation
  # surface (anyone able to drop a .nix file there gets a root rebuild)
  # and an ergonomic hazard (every editor save triggers an activation).
  # Kept as a reference; re-enable by removing the /* */ wrapper.
  /*
