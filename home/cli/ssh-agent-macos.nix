...
let
  ...
  startAndLoad = pkgs.writeShellScript "ssh-agent-startup" ''
    ...
    # Peer identity key — generated locally by `nix run .#peers-bootstrap`,
    ...
    # bootstrap yet).
    peers_key="${homeDir}/.ssh/id_ed25519_peers"
    if [ -r "$peers_key" ]; then
      "${pkgs.openssh}/bin/ssh-add" "$peers_key" </dev/null
    else
      echo "ssh-load-keys: $peers_key missing (run: nix run .#peers-bootstrap)"
    fi
