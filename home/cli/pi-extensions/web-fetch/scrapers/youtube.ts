...
import { ensureTool, globByPrefix, ptree, settings, Snowflake, throwIfAborted } from "../compat";
// `../parallel` is omp's Parallel.ai web-extract integration; not vendored.
// Stubs below keep the scraper compiling and disable the Parallel code path.
interface ParallelExtractResult { results?: unknown[] }
const extractWithParallel = async (
  ..._args: unknown[]
): Promise<ParallelExtractResult | null> => null;
const findParallelApiKey = (): string | undefined => undefined;
const getParallelExtractContent = (_v: unknown): string => "";
	...
	try {
		...
		if (hasManualSubs) {
			...
			if (subResult.ok) {
				...
				const subFiles = await globByPrefix(tmpBase, ".vtt");
				if (subFiles.length > 0) {
					const vttContent = await fs.readFile(subFiles[0], "utf8");
