...
import { $env, getAgentDbPath, readSseJson } from "../compat";
import { AgentStorage } from "../compat";
import type {
...
async function findOAuthToken(): Promise<string | null> {
	// Disabled in this fork: the OAuth credential store is provided by
	// omp's `AgentStorage` (bun:sqlite). We stub it as empty so callers
	// fall through to the env-key path (PERPLEXITY_API_KEY).
	return null;
}
